defmodule Kukumo.Feature do
  ...
  - `global_tags` - Allow you to specify ex unit tag assigned to all kukumo generated tests
  ...
  Given a feature file, create a corresponding feature module which references it. Heres an example:
      ...
      defmodule MyApp.SomeFeatureTest do
        use Kukumo.Feature, file: "some_feature.feature"
  ...
  Then inside the test file (the .exs one) add a `import_feature MyApp.GlobalFeatures` line after the `use Kukumo.Feature` line lke so:
  ...
  """
  import Kukumo.Feature.Helpers

  alias Kukumo.Feature.{Loader, MissingStepError}
  alias Kukumo.DescribeBlock
        ...
        describe describe_block.description do
          setup context do
            for tag <- unquote(scenario.tags) do
              case tag do
                {tag, _value} ->
                  Kukumo.Feature.Helpers.run_tag(
                    unquote(Macro.escape(tags)),
                    tag,
                    __MODULE__,
            ...
            {:ok,
             Map.merge(
               Kukumo.Feature.Helpers.fetch_state(unquote(describe_block.description), __MODULE__),
               context || %{}
             )}
          end

          tags = Kukumo.Feature.Helpers.map_tags(scenario.tags) || []
          ...
          def unquote(name)(exunit_state) do
            Kukumo.Feature.Helpers.start_state(unquote(describe_block.description), __MODULE__, exunit_state)
    ...
    quote generated: true do
      with {_type, unquote(vars)} <- {:variables, unquote(Macro.escape(named_vars))},
           {_type, state = unquote(state_pattern)} <-
             {:state, Kukumo.Feature.Helpers.fetch_state(unquote(scenario_name), __MODULE__)} do
        new_state =
        ...
        Kukumo.Feature.Helpers.update_state(unquote(scenario_name), __MODULE__, fn _ ->
          new_state
        end)
        ...
        {type, state} ->
          ...
          reraise """
                  ** (MatchError) Failure to match #{type} of #{
                    inspect(Kukumo.Feature.Helpers.remove_hidden_state(state))
                  }
                  Pattern: #{unquote(Macro.to_string(state_pattern))}
                  """,
                  Kukumo.Feature.Helpers.stacktrace(__MODULE__, metadata)
      end
    end
  end
  ...
  @doc """
  Brings in all the functionality available from the supplied module. Module must `use Kukumo.Feature` (with or without a `:file`).
  ...
  @doc """
  Brings in all the step definitions from the supplied module. Module must `use Kukumo.Feature` (with or without a `:file`).
  """
  ...
  @doc """
  Brings in all the tag definitions from the supplied module. Module must `use Kukumo.Feature` (with or without a `:file`).
  """
  defmacro import_tags(module) do
    quote do
      if Code.ensure_compiled(unquote(module)) do
        for {name, block} <- unquote(module).raw_tags() do
          Kukumo.Feature.Helpers.add_tag(__MODULE__, name, block)
        end
      end
    end
  end
  ...
  used inside of `Kukumo.Feature`s that don't relate to a file and then imported with `import_feature/1`.
